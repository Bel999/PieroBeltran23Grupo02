...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Compara_modelos_H3_M, 0);
  ...
  Compara_modelos_H3_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(Compara_modelos_H3_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Compara_modelos_H3_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(Compara_modelos_H3_M);
