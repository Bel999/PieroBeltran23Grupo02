...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(controlBA_M, 0);
  ...
  controlBA_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(controlBA_M->extModeInfo, &rtmGetTFinal(controlBA_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(controlBA_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(controlBA_M);
