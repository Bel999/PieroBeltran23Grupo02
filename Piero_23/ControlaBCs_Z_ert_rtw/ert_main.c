...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(ControlaBCs_Z_M, 0);
  ...
  ControlaBCs_Z_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(ControlaBCs_Z_M->extModeInfo, &rtmGetTFinal
    (ControlaBCs_Z_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(ControlaBCs_Z_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(ControlaBCs_Z_M);
