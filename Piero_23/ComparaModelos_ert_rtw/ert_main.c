...
void rt_OneStep(void)
{
   ...
   */
  ComparaModelos_SetEventsForThisBaseStep(eventFlags);
    ...
    switch (2)
    {
     case 2 :
      currentTime = (extmodeSimulationTime_T)
        ((ComparaModelos_M->Timing.clockTick2 * 50) + 0)
        ;
      ComparaModelos_step2();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(ComparaModelos_M, 0);
  ...
  ComparaModelos_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(ComparaModelos_M->extModeInfo, &rtmGetTFinal
    (ComparaModelos_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(ComparaModelos_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(ComparaModelos_M);
