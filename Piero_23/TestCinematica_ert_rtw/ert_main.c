...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(TestCinematica_M, 0);
  ...
  TestCinematica_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(TestCinematica_M->extModeInfo, &rtmGetTFinal
    (TestCinematica_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(TestCinematica_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(TestCinematica_M);
