...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(ControlaBCs_M, 0);
  ...
  ControlaBCs_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(ControlaBCs_M->extModeInfo, &rtmGetTFinal
    (ControlaBCs_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(ControlaBCs_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(ControlaBCs_M);
