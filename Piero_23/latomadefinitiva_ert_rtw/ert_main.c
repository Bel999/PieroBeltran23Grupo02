...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(latomadefinitiva_M, 0);
  ...
  latomadefinitiva_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(latomadefinitiva_M->extModeInfo, &rtmGetTFinal
    (latomadefinitiva_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(latomadefinitiva_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(latomadefinitiva_M);
