...
void rt_OneStep(void)
{
   ...
   */
  ComparaModelos111_SetEventsForThisBaseStep(eventFlags);
    ...
    switch (2)
    {
     case 2 :
      currentTime = (extmodeSimulationTime_T)
        ((ComparaModelos111_M->Timing.clockTick2 * 50) + 0)
        ;
      ComparaModelos111_step2();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(ComparaModelos111_M, 0);
  ...
  ComparaModelos111_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(ComparaModelos111_M->extModeInfo, &rtmGetTFinal
    (ComparaModelos111_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(ComparaModelos111_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(ComparaModelos111_M);
