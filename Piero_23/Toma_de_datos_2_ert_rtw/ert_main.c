...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(toma_de_datos_2_M, 0);
  ...
  toma_de_datos_2_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(toma_de_datos_2_M->extModeInfo, &rtmGetTFinal
    (toma_de_datos_2_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(toma_de_datos_2_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(toma_de_datos_2_M);
