...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) H4_Giro360P_M->Timing.clockTick0;
  H4_Giro360P_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(H4_Giro360P_M, 0);
  ...
  H4_Giro360P_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(H4_Giro360P_M->extModeInfo, &rtmGetTFinal
    (H4_Giro360P_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(H4_Giro360P_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(H4_Giro360P_M);
