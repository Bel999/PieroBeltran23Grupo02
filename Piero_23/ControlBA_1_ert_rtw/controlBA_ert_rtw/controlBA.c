...
{
  {
     ...
     */
    controlBA_B.Gain[0] = controlBA_P.Gain_Gain * (real_T)controlBA_B.Encoder;
    controlBA_B.Gain[1] = controlBA_P.Gain_Gain * (real_T)controlBA_B.Encoder1;

    /* Derivative: '<S3>/Derivative' */
    deltaT = controlBA_M->Timing.t[0];
    if ((controlBA_DW.TimeStampA >= deltaT) && (controlBA_DW.TimeStampB >=
         deltaT)) {
      /* Derivative: '<S3>/Derivative' */
      ...
      if (controlBA_DW.TimeStampA < controlBA_DW.TimeStampB) {
        if (controlBA_DW.TimeStampB < deltaT) {
          lastTime = controlBA_DW.TimeStampB;
          lastU = &controlBA_DW.LastUAtTimeB;
        }
      } else if (controlBA_DW.TimeStampA >= deltaT) {
        lastTime = controlBA_DW.TimeStampB;
        lastU = &controlBA_DW.LastUAtTimeB;
      }
      ...
      /* Derivative: '<S3>/Derivative' */
      controlBA_B.Derivative[0] = (controlBA_B.Gain[0] - (*lastU)[0]) / deltaT;
      controlBA_B.Derivative[1] = (controlBA_B.Gain[1] - (*lastU)[1]) / deltaT;
    }
     ...
     */
    deltaT = floor(look1_binlxpw(controlBA_B.Constant[0],
    ...
    rtb_DataTypeConversion_idx_0 = deltaT < 0.0 ? -(int16_T)(uint16_T)-deltaT :
      (int16_T)(uint16_T)deltaT;
    deltaT = floor(look1_binlxpw(controlBA_B.Constant[1],
    ...
    if (rtb_DataTypeConversion_idx_1 > controlBA_P.Saturation2_UpperSat) {
      y = controlBA_P.Saturation2_UpperSat;
    } else if (rtb_DataTypeConversion_idx_1 < controlBA_P.Saturation2_LowerSat)
    {
      y = controlBA_P.Saturation2_LowerSat;
    } else {
    ...
    /* MATLABSystem: '<S4>/Adelante_der' */
    MW_PWM_SetDutyCycle(controlBA_DW.obj_k.PWMDriverObj.MW_PWM_HANDLE, (real_T)y);
    ...
    if (rtb_DataTypeConversion_idx_1 > controlBA_P.Saturation3_UpperSat) {
      y = controlBA_P.Saturation3_UpperSat;
    } else if (rtb_DataTypeConversion_idx_1 < controlBA_P.Saturation3_LowerSat)
    {
      y = controlBA_P.Saturation3_LowerSat;
    } else {
      y = rtb_DataTypeConversion_idx_1;
    }
    ...
    /* MATLABSystem: '<S4>/Atras_der' */
    MW_PWM_SetDutyCycle(controlBA_DW.obj_l.PWMDriverObj.MW_PWM_HANDLE, (real_T)y);
...
void controlBA_initialize(void)
{
  ...
  rtmSetTFinal(controlBA_M, -1);
  controlBA_M->Timing.stepSize0 = 0.2;

  /* External mode info */
  controlBA_M->Sizes.checksums[0] = (2896131940U);
  controlBA_M->Sizes.checksums[1] = (1982772116U);
  controlBA_M->Sizes.checksums[2] = (2960912309U);
  controlBA_M->Sizes.checksums[3] = (1024579925U);
