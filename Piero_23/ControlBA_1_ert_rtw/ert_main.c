...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(ControlBA_1_M, 0);
  ...
  ControlBA_1_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(ControlBA_1_M->extModeInfo, &rtmGetTFinal
    (ControlBA_1_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(ControlBA_1_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(ControlBA_1_M);
