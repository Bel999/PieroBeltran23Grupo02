...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Toma_De_Datos_grafica_M, 0);
  ...
  Toma_De_Datos_grafica_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(Toma_De_Datos_grafica_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Toma_De_Datos_grafica_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(Toma_De_Datos_grafica_M);
  ...
  /* Terminate model */
  Toma_De_Datos_grafica_terminate();
