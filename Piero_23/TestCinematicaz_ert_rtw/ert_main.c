...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(TestCinematicaz_M, 0);
  ...
  TestCinematicaz_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(TestCinematicaz_M->extModeInfo, &rtmGetTFinal
    (TestCinematicaz_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(TestCinematicaz_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(TestCinematicaz_M);
